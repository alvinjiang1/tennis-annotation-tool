...
def convert_boxes_to_json(boxes, pred_phrases, confidence_scores, image_size):
    """Convert boxes to JSON format with absolute coordinates and labels."""
    W, H = image_size
    json_boxes = []

    for box, label, conf in zip(boxes, pred_phrases, confidence_scores):
        ...
        # Remove confidence score from label if it exists
        # Labels might be in format "label(0.95)" from get_grounding_output
...
def process_images_in_folder(args):
    ...
    for image_path in tqdm(image_files, desc="Processing images"):
        try:
            ...
            # Convert detections to JSON format - now including pred_phrases
            json_boxes = convert_boxes_to_json(boxes_filt, pred_phrases, confidence_scores, image_pil.size)
